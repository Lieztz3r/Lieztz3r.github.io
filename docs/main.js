...
function finishLoading() {
    ...
    loadingDone = true;
    setTimeout(() => {
        loader === null || loader === void 0 ? void 0 : loader.classList.add('fade-out');
        topbar === null || topbar === void 0 ? void 0 : topbar.classList.add('visible');
        document.body.classList.add('loaded');
        setTimeout(() => { if (loader)
...
(function initHeroCarousel() {
    ...
    Array.from(track.querySelectorAll('.carousel-slide')).forEach(s => {
    ...
    function realIdx() {
        if (pos <= 0)
            return REAL - 1; // on clone-of-last → last real
        if (pos >= REAL + 1)
            return 0; // on clone-of-first → first real
    ...
    track.addEventListener('transitionend', (e) => {
        ...
        requestAnimationFrame(() => {
            ...
            if (pos <= 0)
                pos = REAL; // clone-of-last → last real
            else if (pos >= REAL + 1)
                pos = 1; // clone-of-first → first real
            moveTo(pos, false);
            syncDots();
    ...
    prevBtn === null || prevBtn === void 0 ? void 0 : prevBtn.addEventListener('click', () => goTo(pos - 1));
    nextBtn === null || nextBtn === void 0 ? void 0 : nextBtn.addEventListener('click', () => goTo(pos + 1));
    moveTo(pos, false);
    syncDots();
...
/* ═══════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════ */
function makeRevealObserver(threshold = 0.15, delay = 0) {
...
const contentImage2 = document.getElementById('contentImage2');
...
/* ═══════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════ */
const floatImgWrap = document.getElementById('floatImgWrap');
const floatSection = document.getElementById('float-section');
if (floatImgWrap && floatSection) {
    const inner = floatImgWrap.querySelector('.float-img-inner');
    new IntersectionObserver(entries => {
        if (entries[0].isIntersecting)
            floatImgWrap.classList.add('in-view');
    }, { threshold: 0.2 }).observe(floatImgWrap);
    let lastY = window.scrollY, pY = 0, tY = 0, raf = null;
    ...
    function tick() {
        ...
        tY = lerp(tY, 0, 0.06);
        if (inner)
            inner.style.transform = `translateY(${pY.toFixed(2)}px)`;
        if (Math.abs(pY) > 0.3 || Math.abs(tY) > 0.3) {
...
(function () {
    ...
    ids.forEach(id => { const el = document.getElementById(id); if (el)
        o.observe(el); });
    imgIds.forEach(id => { const el = document.getElementById(id); if (el)
        oi.observe(el); });
})();
/* ═══════════════════════════════════════════════
...
(function initNewsletter() {
    ...
    if (!emailInput || !submitBtn || !msgEl)
        return;
    function buildBody(to) {
    ...
    async function doSubscribe() {
        ...
        try {
            ...
            if (res.ok) {
                msgEl.textContent = '¡Listo! Revisa tu correo para continuar.';
                emailInput.value = '';
    ...
    emailInput.addEventListener('keydown', (e) => { if (e.key === 'Enter')
        void doSubscribe(); });
})();
/* ═══════════════════════════════════════════════
...
(function initServicesCarousels() {
    ...
    function initCarousel(section) {
        ...
        const dotsWrap = section.querySelector('.svc-dots');
        const prevBtn = section.querySelector('.svc-nav-btn.svc-prev');
        const nextBtn = section.querySelector('.svc-nav-btn.svc-next');
        if (!stage || !track || !dotsWrap)
            return;
        const realCards = Array.from(track.querySelectorAll('.svc-card'));
        const REAL = realCards.length;
        if (REAL === 0)
            return;
        /* ── Prepend clones of last N_CLONES real cards ── */
        for (let i = N_CLONES - 1; i >= 0; i--) {
            const hc = realCards[REAL - 1 - (i % REAL)].cloneNode(true);
        ...
        for (let i = 0; i < N_CLONES; i++) {
            const tc = realCards[i % REAL].cloneNode(true);
    ...
    document.querySelectorAll('.svc-carousel-section').forEach(initCarousel);
})();
